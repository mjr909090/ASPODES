...
auditAnnualSummaryApp.controller('ngAuditAnnualSummaryCtrl', ['$scope', '$http', '$downloadFile', function ($scope, $http, $downloadFile) {
    var assignBookUrl = "/api/annualTask/instAdmin/review";//Get获取列表
    var leaderInfoUrl = "/api/person/";//Get负责人信息的Url

    var reviewAssignBookUrl = "/api/annualTask/instReviewAnnualReport/"; //put  pass=true提交年度总结；pass=false驳回年度总结
    ...
    $scope.rejectAssignBook = function (id) {
        $http({
         ...
         .success(function (response) {
             if (response.status == 0) {
                 notif({
                     msg: "<b>提示:</b> 该年度任务的年度报告已驳回至项目负责人处",
                     type: "success"
                 });
                 $scope.load_page($scope.p_current); //refresh
             }
