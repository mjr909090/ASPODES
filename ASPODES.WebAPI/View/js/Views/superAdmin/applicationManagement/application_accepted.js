 ...
 */
var application_accepted = angular.module('ngApplication_AcceptedApp', ['publicFunction']);

application_accepted.controller('ngApplication_AcceptedCtrl', function ($scope, $http, $downloadFile) {
    ...
    var personURL = "/api/person/"
    var getAcceptUrl = "/api/departapplication/evaluate/"

    $scope.downloadPDFUrl = "/api/applicationdoc/download/pdf/";//下载PDF的Url

    $scope.exportDeptAcceptedListUrl = "/api/deptapplication/accept"; // 单位管理员待受理申请书导出列表的url

    // 定义
    $scope.exportDeptAcceptedListFields = {};
    $scope.exportDeptAcceptedListName = "已受理申请书";
    ...
    $scope.acceptApplicationId = function (ApplicationId) {
    ...
    //点击负责人得到LeaderId
    $scope.acceptLeaderId = function (LeaderId) {
    ...
    $scope.showModifyApplicationModal = function(){
        $("#modifyApplication").modal("show");
    };
    //显示PDF页面
    ...
    $scope.selectAll = function () {
        if ($scope.select_all) {
            $scope.checked = [];
            angular.forEach($scope.acceptApplications, function (divisionApply) {
                divisionApply.checked = true;
                $scope.checked.push(divisionApply.ApplicationId);
            })
        } else {
            angular.forEach($scope.acceptApplications, function (divisionApply) {
                divisionApply.checked = false;
                $scope.checked = [];
            })
        }
    };
    ...
    $scope.selectSingle = function () {
        angular.forEach($scope.acceptApplications, function (acceptApplication) {
            var index = $scope.checked.indexOf(acceptApplication.ApplicationId);
            if (acceptApplication.checked && index === -1) {
                $scope.checked.push(acceptApplication.ApplicationId);
            } else if (!acceptApplication.checked && index !== -1) {
                $scope.checked.splice(index, 1);
            };
        })

        if ($scope.acceptApplications.length === $scope.checked.length) {
    ...
    //获取已受理申请书列表  api/departapplication/evaluate/{instituteId}/{projectTypeId}/{pageId}
    $scope.getInstIdForPage = function (InstituteId) {//单位
        ...
        // $scope.getAccpetApplicationList();
    ...
    $scope.load_page = function (page) {
        ...
        var acceptApplicationList = getAcceptUrl + thisInstituteId + "/" + thisProjectTypeId + "/" + page;
       
        $http.get(acceptApplicationList)
            .success(function (response) {
                if (response.response != null) {
                    $scope.acceptApplications = response.response.ItemDTOs;
                    console.log($scope.acceptApplications);
                    ...
                    if ($scope.acceptApplications.length == 0) {
    ...
    $scope.searchContent = function (searchPage) {
        ...
        var acceptApplicationList = getAcceptUrl + thisInstituteId + "/" + thisProjectTypeId + "/" + searchPage;
    ...
    // 导出列表
    $scope.exportDeptAcceptedList = function (ProjectTypeId, InstituteId) {
        if (ProjectTypeId == undefined) {
            $scope.ProjectTypeId = 0;
        } else {
            $scope.ProjectTypeId = ProjectTypeId;
        ...
        $downloadFile.exportApplication($scope.exportDeptAcceptedListUrl, $scope.exportDeptAcceptedListFields, $scope.exportDeptAcceptedListName);
    };
   ...
   */
    ...
    /*
     $scope.getAccpetApplicationList = function () {
        if ($scope.InstituteId == undefined) {//单位
            thisInstituteId = '0'
        } else {
            thisInstituteId = $scope.InstituteId
        ...
        var acceptApplicationList = getAcceptUrl + thisInstituteId + "/" + thisProjectTypeId + "/" + "1";
        $http.get(acceptApplicationList)
               .success(function (response) {
                   $scope.acceptApplications = response.response.ItemDTOs;
                   $scope.pageItem = response.response.TotalPageNum;//总共多少页
               ...
               .error(function () {
                   notif({
                       msg: "<b>Error:</b> 已受理申请书列表未能加载，请重试",
                       type: "error"
                   });
               });
    }
    $scope.getAccpetApplicationList();//启动方法
    //选择首页
    $scope.p_index = function () {
        ...
        var acceptApplicationList = getAcceptUrl + thisInstituteId + "/" + thisProjectTypeId + "/" + "1";
       // var acceptApplicationList = getAcceptUrl + "0/" + "0/" + "1";
        $http.get(acceptApplicationList)
              .success(function (response) {
                  $scope.acceptApplications = response.response.ItemDTOs;
                  $scope.pageItem = response.response.TotalPageNum;//总共多少页
    ...
    $scope.p_prev = function () {
        ...
        var acceptApplicationList = getAcceptUrl + thisInstituteId + "/" + thisProjectTypeId + "/" + $scope.p_current;
        //var acceptApplicationList = getAcceptUrl + "0/" + "0/" + $scope.p_current;
        $http.get(acceptApplicationList)
              .success(function (response) {
                  $scope.acceptApplications = response.response.ItemDTOs;
                  $scope.pageItem = response.response.TotalPageNum;//总共多少页
    ...
    $scope.p_last = function () {
        ...
        var acceptApplicationList = getAcceptUrl + thisInstituteId + "/" + thisProjectTypeId + "/" + $scope.pageItem;
        $http.get(acceptApplicationList)
              .success(function (response) {
                  $scope.acceptApplications = response.response.ItemDTOs;
                  $scope.pageItem = response.response.TotalPageNum;//总共多少页
