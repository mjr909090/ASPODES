...
divisionPaticipateApp.controller('ngDivisionPaticipateCtrl', ['$scope', '$http', '$downloadFile', function ($scope, $http, $downloadFile) {
    ...
    $scope.downloadPDF = function (AnnualTaskId, taskName) {
        var taskPDFUrl = "/api/annualTaskDoc/" + AnnualTaskId + "/pdf";
        $downloadFile.downloadPDF(taskPDFUrl, AnnualTaskId, taskName);
    }
        ...
        template:
            '<div>' +
                '<ul class="nav nav-tabs">' +
                    '<li class="nav-item" ng-repeat="tab in tabs.tabs">' +
                        '<a class="nav-link" ng-click="tabs.selectTab($index);" ng-bind="tab.label"></a>' +
                    '</li>' +
                '</ul>' +
